<template>
  ...
  <div>
    <head-top head-title="用电日趋势图" go-back='true'>  </head-top>
...
<script>
  ...
  import {getDayList} from 'src/service/getData'
  ...
  export default {
    ...
    methods: {
      onParamChange (attr, val) {
        ...
        // lineCharts.getChart().xAxis[0].setTickPixelInterval
        var title = '';
        ...
        if(this.itemStatus == 0){
          ...
          this.getDayTime(lineCharts,'温度B');
          this.itemType="temperatureC";
          this.getDayTime(lineCharts,'温度C');
          this.itemType="temperatureN";
          this.getDayTime(lineCharts,'温度N');
          ...
          this.getDayTime(lineCharts,'电流B');
          this.itemType="currentC";
          this.getDayTime(lineCharts,'电流C');
          ...
          this.getDayTime(lineCharts,'电压B');
      ...
      getRealTime(lineCharts){
        getDayList(this.sbbId,this.dateTo,this.itemType).then(res => {
      ...
      async  getDayTime(lineCharts,title){
        await getDayList(this.sbbId,this.dateTo,this.itemType).then(res => {
          this.placelist=[];
          this.sbbName=res.name;
          for(var i=0;i<res.data.length;i++){
